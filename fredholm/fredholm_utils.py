...
def estimated_b_function_matrix(range_linspace, init_data, kernel, known_b, y_domain, int_N=5000, chunk_size=100):
    ...
    for start_idx_i in range(0, n_total, chunk_size):
        ...
        b_y_chunk = known_b(np.asarray(linspace_vals, dtype=np.float32)[start_idx_i:end_idx_i], y_z[:, 0]).astype(np.float32)
        ...
        for start_idx_j in range(0, n_total, chunk_size):
            ...
            b_z_chunk = known_b(np.asarray(init_data, dtype=np.float32)[start_idx_j:end_idx_j], y_z[:, 1]).astype(np.float32)
...
def estimated_b_function_mat_calc(b_ij, betas):
    # now we use the betas and multiply them into the matrix along the columns axis (since j is columns, and each beta (or c) corresponds to a j value
...
def estimated_b_function_vect(val, init_data, kernel, known_b, y_rvs, int_N=5000):
    ...
    b_z = known_b(np.asarray(init_data, dtype=np.float32), y_z[:, 1]).astype(np.float32) #shape (n, N)
...
# does not include plotting or calculating errors (that is left for later classes/functions/code)
class FredholmGlobLoc:
    ...
    def get_mu_vector(self, c_mat, b_int, d_mat, theta):
    ...
    def run_gibbs(self, verbose=True, matrix_calc='chunked'):
        ...
        for i in range(self.gibbs_iters):
            ...
            np.fill_diagonal(lambda_mat, [gig_rvs(a_loc, (1/tau)*(beta[i]**2+b_loc), p_loc - 1/2 - 1, 1) for i in range(data_len - 1)])

            # lambda then informs the distribution of tau
            tau = gig_rvs(a_glob, np.sum([(1/lamb)*(beta[i]**2) for i, lamb in enumerate(np.diagonal(lambda_mat))]) + b_glob, p_glob - 1/2 - 1, 1)
...
class FredholmOptimize:
    ...
    def optimize_c(self, matrix_calc='chunked', **kwargs):
        ...
        dt = self.t_delta
        def f(c):
            part1 = dt*(c.T@b_int.T@D@b_int@c)
            part2 = 2*(theta.T@D@b_int@c)
            part3 = np.linalg.norm(c)
            return part1 - part2 + part3

        min_c = minimize(f, np.zeros(len(D)), **kwargs)
