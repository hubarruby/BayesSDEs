...
def parse_args():
    ...
    parser.add_argument('--results_filepath', type=str, required=True, help='filepath to save the results (should be a .pkl).')
    ...
    parser.add_argument('--y_domain', type=float, nargs=2, default=[-1000, 1000], help='Domain for the Y variable as low, high.')
    parser.add_argument('--b_function', type=str, choices=['b1_v', 'b2_v'], default='b1_v', help='The b function to use in the simulation (b1_v or b2_v).')
    parser.add_argument('--local_gig_params', type=float, nargs=3, default=[2,0,1], help='Parameters for the local GIG distribution as a list: shape, skew, scale.')
    parser.add_argument('--global_gig_params', type=float, nargs=3, default=[2,0,1], help='Parameters for the global GIG distribution as a list: shape, skew, scale.')
    parser.add_argument('--kernel_name', type=str, default='gauss', help='Name of the kernel function used in calculations.')
...
class FredSimulation:
    def __init__(self, results_filepath, scale=np.sqrt(10), loc=1, a_trunc=-100, b_trunc=100,
                 ...
                 y_domain=(-100, 100), b=b1_v, local_gig_params=[2,0,1], global_gig_params=[2,0,1],
                 kernel_name = 'gauss'):
        self.scale = scale
        self.loc = loc
        ...
        self.b = b
        self.local_gig_params = local_gig_params
        self.global_gig_params = global_gig_params
    ...
    def run_simulation(self):
        ...
        init_data = create_data(self.diffusion, b_bar, t_delta=self.t_delta, t_end=self.t_end, start_val=self.start_val, pi=pi, b=self.b, integral_N=self.integral_N)
        ...
        fred_glob_loc_obj = FredholmGlobLoc(init_data=init_data,
                                            ...
                                            local_gig_params=self.local_gig_params,
                                            global_gig_params=self.global_gig_params,
        ...
        fred_results_dict = {
            ...
            'init_data': fred_glob_loc_obj._init_data,
...
def main():
    ...
    simulation = FredSimulation(
        ...
        b=b_func,
        local_gig_params=args.local_gig_params,
        global_gig_params=args.global_gig_params,
